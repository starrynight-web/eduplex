...
// Exam Routine Dropdown Links (commented out since element doesn't exist in HTML)
/*
...
// Countdown Timer - Auto-switching between single exam and schedule
const examSchedule = [
  new Date("October 29, 2025 09:00:00").getTime(), // Exam 1
  new Date("October 31, 2025 09:00:00").getTime(), // Exam 2
  new Date("November 02, 2025 09:00:00").getTime(), // Exam 3
  new Date("November 04, 2025 09:00:00").getTime(), // Exam 4
];

function getNextExam() {
  const now = new Date().getTime();
  // Find the first exam date that hasn't started yet
  return examSchedule.find((date) => date > now) || null;
}

function getCurrentExamIndex() {
  const now = new Date().getTime();
  // Find the index of the currently ongoing exam
  for (let i = 0; i < examSchedule.length; i++) {
    const examStart = examSchedule[i];
    const examEnd = examStart + 3 * 60 * 60 * 1000; // Assuming 3-hour exams
...
// Enhanced version with exam names (optional)
const examScheduleWithNames = [
  {
    date: new Date("October 29, 2025 09:00:00").getTime(),
    name: "Bangladesh Studies",
  },
  {
    date: new Date("October 31, 2025 09:00:00").getTime(),
    name: "Computer Fundamentals",
  },
  {
    date: new Date("November 02, 2025 09:00:00").getTime(),
    name: "Software Engineering",
  },
  {
    date: new Date("November 04, 2025 09:00:00").getTime(),
    name: "English - 1",
  },
];

function getNextExamEnhanced() {
  const now = new Date().getTime();
  return examScheduleWithNames.find((exam) => exam.date > now) || null;
}

function getCurrentExamIndexEnhanced() {
  const now = new Date().getTime();
  for (let i = 0; i < examScheduleWithNames.length; i++) {
    const examStart = examScheduleWithNames[i].date;
...
function updateCountdownWithNames() {
  const now = new Date().getTime();
  const nextExam = getNextExamEnhanced();
  const currentExamIndex = getCurrentExamIndexEnhanced();
  const subtitleElement = document.querySelector(".countdown-subtitle");

  if (currentExamIndex !== -1) {
    // Exam ongoing
    const currentExam = examScheduleWithNames[currentExamIndex];
    const examEnd = currentExam.date + 3 * 60 * 60 * 1000;
    const timeUntilEnd = examEnd - now;

    updateDisplay(timeUntilEnd);
    subtitleElement.textContent = `${currentExam.name} ends in:`;
  } else if (nextExam) {
    // Counting down to next exam
    const timeUntilStart = nextExam.date - now;
    updateDisplay(timeUntilStart);
    subtitleElement.textContent = `${nextExam.name} starts in:`;
  } else {
    // All exams finished
    document.getElementById("days").textContent = "00";
    document.getElementById("hours").textContent = "00";
    document.getElementById("minutes").textContent = "00";
    document.getElementById("seconds").textContent = "00";
    subtitleElement.textContent = "All exams are finished!";
  }
}
...
// For simple version (just exam numbers):
updateCountdown();
setInterval(updateCountdown, 1000);

// For enhanced version (with exam names) - UNCOMMENT BELOW:
// updateCountdownWithNames();
// setInterval(updateCountdownWithNames, 1000);
...
function syncDisabledOptions() {
  ...
  selects.forEach((current) => {
    Array.from(current.options).forEach((opt) => {
      if (!opt.value) return;
      opt.disabled =
        selectedCodes.has(opt.value) && current.value !== opt.value;
    });
  });
}
...
function getGradeAndCgpa(marks) {
  let cgpa = 0;
  if (marks >= 80) cgpa = 4.0;
  else if (marks >= 75) cgpa = 3.75;
  else if (marks >= 70) cgpa = 3.5;
  else if (marks >= 65) cgpa = 3.25;
  else if (marks >= 60) cgpa = 3.0;
  else if (marks >= 55) cgpa = 2.75;
  else if (marks >= 50) cgpa = 2.5;
  else if (marks >= 45) cgpa = 2.25;
  else if (marks >= 40) cgpa = 2.0;
  else cgpa = 0.0;
...
function calculateSemesterCgpa() {
  ...
  for (let i = 1; i <= rowCount; i++) {
    ...
    const isRowEmpty =
      !subjectCode && (isNaN(marks) || marksInput.value.trim() === "");
  ...
  if (!isValid || !anyIncluded || totalCredits === 0) {
    messageEl.textContent =
      "Please complete all subject selections and marks inputs.";
    resultEl.textContent = "--";
    return;
  }
...
function refreshSubjectInputs() {
  ...
  const subjectRows = Array.from(form.querySelectorAll(".form-group")).filter(
    (el) =>
...
document
  .getElementById("refresh-subjects-btn")
  .addEventListener("click", refreshSubjectInputs);
